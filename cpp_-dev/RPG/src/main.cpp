...
class Character
{
  ...
  public:
    ...
    void
    takeDamage (int takeDamage_value)
    {
        hp -= takeDamage_value;
...
main ()
{
    ...
    while (!fighters[0]->isDead ()) // if_alive
        {
            ...
            if (!fighters[static_cast<size_t> (targetIndex)]
                     ->isDead ()) // if_alive
                {
                    fighters[0]->attack (
                        *fighters[static_cast<size_t> (targetIndex)]);
                }
            else
                {
    ...
    if (fighters[0]->isDead ()) // if_dead
        {
            std::cout << "Victory! The Goblin King has been defeated!"
                      << std::endl;
        }
    else
        {
